...
class car:
    ...
    def __init__(self,canvas,x,y,psi,factor,color):
        ...
        self.c5 = (x+self.cl/2, y) # create light_center of car
    ...
    def car_rot_d(self,dpsi): # only rotation difference
        ...
        cc_lc = np.array([l_c[0] - c_c[0],l_c[1] - c_c[1]]) # vector from car_center to light_center
        ...
        cc_lcn = np.dot(rot, cc_lc) # vector from car_center to rotated light_center
        ...
        self.canvas.delete(self.light_center) # delete old light_center
        lc = (cc_lcn[0] + c_c[0], cc_lcn[1] + c_c[1]) # update vector from canvas_GUI to light_center
        self.light_center = canvas.create_bitmap(lc) # set new light_center_position
    ...
    def car_rot(self,psi): # only rotation
        ...
        lc = (c_c[0]+self.cl/2, c_c[1]) # horizontal light_center_position   
