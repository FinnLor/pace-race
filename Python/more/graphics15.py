...
class car:
    ...
    def __init__(self,canvas,x,y,psi,delta,FACTOR,color):
        ...
        self.c4 = (x+self.cl/2, y) # create light_center of car
        self.c5 = (x+self.cl/2, y+self.cw/2) # create upper right corner of car
        self.s1 = (self.c4[0], self.c4[1]-self.cw*3/4)
        
        # self.s2 = (self.c4[0], self.c4[1]-self.cw*3/4)
        # self.s3 = (self.c4[0], self.c4[1]-self.cw*3/4)
        # self.s4 = (self.c4[0], self.c4[1]-self.cw*3/4)
        # self.s5 = (self.c4[0], self.c4[1]-self.cw*3/4)
        # self.s6 = (self.c4[0], self.c4[1]-self.cw*3/4)
        # self.s7 = (self.c4[0], self.c4[1]+self.cw*3/4)
    ...
    def car_rot_d(self,dpsi): # only rotation difference
        ...
        cc_c4 = np.array([c_p[6] - c_c[0],c_p[7] - c_c[1]]) # vector from car_center to c4 (light_center)
        ...
        cc_c4n = np.dot(rot, cc_c4) # vector from car_center to rotated c4 (light_center)
        ...
        self.c4 = (cc_c4n[0] + c_c[0], cc_c4n[1] + c_c[1]) # update vector from canvas_GUI to c4 (light_center)
    ...
    def car_rot(self,psi): # only rotation
        ...
        self.c4 = (c_c[0]+self.cl/2,  c_c[1]) # set light center of car
        self.c5 = (c_c[0]+self.cl/2,  c_c[1]+self.cw/2) # set upper right corner of car
       
     
        # get vectors of corner
        ...
        cc_c4 = np.array([self.c4[0] - c_c[0],self.c4[1] - c_c[1]]) # vector from car_center to c4 (light_center)
        ...
        self.c5 = (cc_c5n[0] + c_c[0], cc_c5n[1] + c_c[1]) # new polygon vector from canvas_GUI to c4
    ...
    def set_car_pos(self,x,y,psi,delta): # set new car postion and angles (with zero velocities)
        ...
        self.c4 = (x+self.cl/2, y) # light_center of car
    ...
    def car_pos_reset(self): # reset car position
        ...
        self.c4 = ( self.cl/2,  0) # light_center of car
...
class track:
    ...
    def __init__(self,canvas,FACTOR,WIDTH,HEIGHT,NPOINTS):
        ...
        # GENERATE TRACK AND BORDER    
        line_data  = list((np.ravel(([x,y]),'F'))) # list is neccessary for a correct separation with comma
        track_center_ls = LineString(np.reshape(line_data,(self.NPOINTS,2)))
        track_right_line1 = np.array(track_center_ls.parallel_offset(15,"right",join_style=1))
        track_right_line = list((np.ravel(([track_right_line1[:,0],track_right_line1[:,1]]),'F')))
        track_left_line1 = np.array(track_center_ls.parallel_offset(15,"left",join_style=1))
        track_left_line = list((np.ravel(([track_left_line1[:,0],track_left_line1[:,1]]),'F')))
        
        # ASSIGN TRACKS TO OBJECT
        self.track_center_line = canvas.create_line(line_data, dash = (4), fill = "grey")
        self.track_right_line   = canvas.create_line(track_right_line, fill = "brown")
        self.track_left_line   = canvas.create_line(track_left_line, fill = "brown")
        
        canvas.create_line
        #xxx = track.simplify(1,True) # can be very usefull!!



    def get_track_line(self):
        # print ("get_track_line")
        
        track_centerline = self.canvas.coords(self.track_center_line) # extract car_center position data
...
car01 = car(canvas, 140, 100,  0.4, 0.4, FACTOR, "green")
car02 = car(canvas, 100, 200,  0.6, 0.6, FACTOR, "blue")
car03 = car(canvas, 110,  90,    0,   0, FACTOR, "orange")
car04 = car(canvas, 120, 100,    1,   1, FACTOR, "black")
car05 = car(canvas, 500, 400,  1.3, 1.3, FACTOR, "white")
car06 = car(canvas, 300, 500,  1.3, 1.3, FACTOR, "brown")
car07 = car(canvas, 140,  10,  0.4, 0.4, FACTOR, "yellow")
car08 = car(canvas, 150,  50,  0.6, 0.6, FACTOR, "olive")
car09 = car(canvas, 110,  90,    0,   0, FACTOR, "cyan")
car10 = car(canvas, 120, 100,    1,   1, FACTOR, "red")
car11 = car(canvas, 200, 300,  1.3, 1.3, FACTOR, "purple")
car12 = car(canvas, 200, 500,  1.3, 1.3, FACTOR, "pink")
     
### PREPARE TRACK FOR DISTANCE-MEASURE
# xxx hier weiter
track_centerline = c_track.get_track_line()
track_data = np.reshape(track_centerline, (NPOINTS,2))
